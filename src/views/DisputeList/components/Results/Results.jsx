...
import { DisputeCard, Paginate } from '../../../../components';
...
const ProjectsGrig = (props) => {
  ...
  if (rootStore.disputes.getDisputes) {
    return (
      <Grid
      container
      spacing={3}
      >
        {projects.map(project => (
          <Grid
            item
            key={project ? project.task.uuid : getRandomArbitary(100, 666)}
            md={mode === 'grid' ? 4 : 12}
            sm={mode === 'grid' ? 6 : 12}
            xs={12}
          >
            <DisputeCard project={project ? project : ""} rootStore={rootStore} />
          </Grid>
        ))}
      </Grid>
    )
